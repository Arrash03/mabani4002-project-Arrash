from tkinter import messagebox, Button, DISABLED, Label
...
class Cell:
    ...
    def createcell(self, frame):
        global game_frame
        game_frame = frame
        btn = Button(frame, text="", width=4, height=2, font=font.Font(family="Cabinet Grotesk", size=10, weight="bold"))
    ...
    def randomize_mines(self):
        un_mine_cells = [self.surrounded_cells(self.i + i, self.j + j) for i in [-1, 0, 1] for j in [-1, 0, 1]
                         if self.surrounded_cells(self.i + i, self.j + j) is not None]
    ...
    def show_cell(self):
        ...
        if counter != 0:
            ...
            self.cell_btn_object.config(bg="#FDAE38")
            # self.empty_cell()
        self.is_opened = True
    ...
    def show_mine(self):
        self.cell_btn_object.config(state=DISABLED)
        self.cell_btn_object.config(text="\U0001F4A3", bg="#FD5B5F")
        for cell in Cell.all_cell:
            ...
            if cell.is_mine:
                cell.cell_btn_object.config(text="\U0001F4A3", bg="#FD5B5F")
                sleep(0.1)
                game_frame.update()
        sleep(0.5)
        messagebox.showwarning(message="You clicked on a Mine!!", title="Game Over")
    ...
    def right_click(self, event):
        if not self.is_flag:
            if Cell.flag_number > 0:
                ...
                self.cell_btn_object.config(text="\U0001F3F3", bg="#7CB3D0", fg="white", state=DISABLED)
                self.cell_btn_object.unbind("<Button-1>")
        else:
            self.is_flag = False
            Cell.flag_number += 1
            Cell.flag_lbl.config(text=f"Flag : {Cell.flag_number}")
            self.cell_btn_object.config(text="", bg="SystemButtonFace", fg="black", state=font.NORMAL)
    ...
    def first_click(self):
        ...
        self.show_cell()
        #
        for btn in Cell.all_cell:
            btn.cell_btn_object.bind("<Button-3>", btn.right_click)
        self.cell_btn_object.unbind("<Button-3>")
        #
        self.empty_cell()
        Cell.mouse_click += 1
    ...
    def create_lbl(cls, frame):
        Cell.cell_lbl = Label(frame, text=f"Cell : {Gridsize_width * Gridsize_height}", bg="#264248", fg="#FDAE38",
                              font=("Cabinet Grotesk", 15))
        Cell.cell_lbl.place(x=5, y=height_percent(10))
        Cell.cell_number = Gridsize_width * Gridsize_height
        Cell.flag_lbl = Label(frame, text=f"Flag : {mine_number}", bg="#264248", fg="#FDAE38",
                              font=("Cabinet Grotesk", 15))
